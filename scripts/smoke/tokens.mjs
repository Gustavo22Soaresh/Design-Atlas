...
const packageName = "@design-atlas/tokens";
const tempDir = mkdtempSync(join(tmpdir(), "smoke-tokens-"));

// Read actual package version from package.json
const packageJsonPath = join(process.cwd(), "packages/tokens/package.json");
...
ReactDOM.createRoot(document.getElementById('root')!).render(
  ...
  // Create App.tsx with tokens import
...
function App() {
  return (
    <div style={{ 
      ...
      <h1>Tokens Smoke Test</h1>
      <p>Tokens loaded successfully!</p>
...
`;
  ...
  console.log("✅ Tokens smoke ok");
} catch (error) {
  console.error("❌ Tokens smoke test failed:", error.message);
  process.exit(1);
} finally {
